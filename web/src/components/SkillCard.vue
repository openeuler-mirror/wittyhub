...
<template>
  <RouterLink
    ...
    class="card block hover:border-primary-200"
  >
    <div class="flex items-start justify-between gap-4">
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-1">
          <h3 class="text-lg font-medium text-gray-900 truncate">{{ skill.name }}</h3>
          <SecurityBadge v-if="skill.security_score !== null" :score="skill.security_score" />
        </div>
        <p class="text-sm text-gray-500 mb-3">{{ skill.skill_id }}</p>
        ...
        <div class="flex flex-wrap items-center gap-2">
          <span
            v-if="skill.category"
            class="px-2 py-0.5 bg-primary-50 text-primary-600 text-xs rounded-full"
          >
            {{ skill.category }}
          </span>
          <span
